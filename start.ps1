...
# Use the existing Camoufox runtime; do not download browser binaries during
# normal startup.
...
# Keep the Web UI available while the optional browser-backed solver starts.
$WebAppUp = $false
try {
    $health = Invoke-RestMethod -Uri "http://127.0.0.1:3080/api/health" -TimeoutSec 2
    $WebAppUp = $health.service -eq "progrok-registration"
} catch {}
...
if (-not $SolverUp) {
    # Start the solver without browser prewarm so the Web service remains
    # available when Camoufox initialization is slow.
    if (-not $env:TURNSTILE_LAZY) { $env:TURNSTILE_LAZY = "1" }
    ...
    if (-not $SolverReady) {
        Write-Warning "Turnstile Solver failed to become ready. The Web UI remains available; check runtime\logs\solver.err.log"
    } else {
