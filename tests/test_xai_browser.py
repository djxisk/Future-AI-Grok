...
class XaiBrowserTests(unittest.TestCase):
    def test_wait_reports_closed_page_as_registration_error(self):
        visual = XaiVisibleRegistration()
        visual.page = _Page()
        visual.page.close()

        with self.assertRaisesRegex(Exception, "browser page closed"):
            visual._wait(200)
    ...
    def test_visible_runtime_disables_background_window_throttling(self):
        ...
        addons_module = types.ModuleType("camoufox.addons")
        addons_module.DefaultAddons = types.SimpleNamespace(UBO="ubo")
        sync_api_module.Camoufox = _Manager
        camoufox_module.sync_api = sync_api_module
        camoufox_module.addons = addons_module
        with (
...
class XaiRegistrationConfigTests(unittest.TestCase):
    ...
    def test_post_registration_keeps_probe_proxy_settings(self):
        ...
        self.assertEqual(config["proxy_username"], "proxy-user")
        self.assertEqual(config["proxy_password"], "proxy-pass")
        self.assertEqual(config["proxy_strategy"], "sticky")
