...
# A blocked upstream can keep the document event open indefinitely.  Commit
# fires once the response headers arrive, letting the state machine inspect the
# error page and terminate the session instead of occupying a worker slot.
    ...
    def ensure(
        ...
        *,
        ...
        try:
            ...
            # A stale empty UBO cache entry makes Camoufox reject startup;
            # browser fingerprinting remains provided by Camoufox itself.
...
class XaiVisibleRegistration:
    ...
    def _wait(self, milliseconds: int) -> None:
        ...
        while remaining > 0:
            ...
            if self.page is None:
                ...
                try:
                    is_closed = bool(self.page.is_closed())
                ...
                if is_closed:
                    raise XaiBrowserError(
                        "xAI registration browser page closed; browser may have crashed or run out of memory"
                    )
                try:
    ...
    def open(self) -> None:
        ...
        try:
            ...
            self._action_delay()
            self.page.goto(
                XAI_SIGNUP_URL, wait_until="commit", timeout=NAVIGATION_TIMEOUT_MS
            )
            self._settle_page()
            self._enforce_visible_window()
